...
def train_worker(
    ...
    train_loader = ShogiDataLoader(
        args.train_data, args.batch, device, format=args.format, shuffle=True, features_mode=args.input_features
    )
    test_loader = ShogiDataLoader(
        args.test_data, args.test_batch, device, format=args.format, features_mode=args.input_features
    )
